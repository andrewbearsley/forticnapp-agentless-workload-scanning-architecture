# Lacework FortiCNAPP - Agentless Workload Scanning Architecture and Deployment Details
...
### Azure Process
1. Container App jobs are scheduled hourly to check for scanning requirements
...
5. Cloned disks are analyzed for vulnerabilities, then deleted
6. Scan results are stored in Storage Account
...
- Container App jobs are scheduled hourly to check for scanning requirements
- Container App jobs enumerate monitored subscriptions and identify Virtual Machines that need scanning
- Azure APIs are used to clone VM disks
- Virtual Machines are launched to mount and scan the cloned disks
- Cloned disks are analyzed for vulnerabilities within the Azure subscription, then deleted after scanning
- Scan results (metadata) are stored in Storage Account within the Azure subscription
- Lacework uses Service Principal with read-only Storage Account access to retrieve scan results
- Container App jobs require outbound internet connectivity to Lacework APIs for configuration updates, diagnostic reporting, and on-demand scan requests
...
The Terraform module provisions both global and regional resources:
...
- Virtual Machines - used to mount and scan cloned disks
...
- Managed Identity - for Container App jobs to access Azure resources

**Deployment Scenarios:**
- Tenant-level (single or multiple regions)
- Subscription-level (single or multiple regions)

Reference: https://registry.terraform.io/modules/lacework/agentless-scanning/azure/latest

### IAM Permissions

#### Permissions Required for Deployment

The Terraform module requires Azure permissions to deploy the scanning infrastructure. The deploying user/service principal needs permissions to create:

- Azure AD Application and Service Principal
- Role Assignments
- Resource Groups
- Container App Environments and Jobs
- Virtual Machines
- Storage Accounts
